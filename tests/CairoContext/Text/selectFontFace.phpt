...
try {
    $context->selectFontFace('sans-serif', Cairo\Font\Slant::NORMAL, Cairo\Font\Weight::NORMAL, 1);
    trigger_error('selectFontFace requires at least one arg');
} catch (Cairo\Exception $e) {
    echo $e->getMessage(), PHP_EOL;
}
...
/* Wrong arg type - int */
try {
    $context->selectFontFace('sans-serif', Cairo\Font\Slant::NORMAL, array());
...
Cairo\Context::selectFontFace() expects parameter 2 to be long, array given
Cairo\Context::selectFontFace() expects parameter 3 to be long, array given